...
/** The mod's own vanilla-registry tags, grouped by registry. Being datapack tags, all of them are
  * overridable/extendable by datapacks and other mods.
  */
object CasualtiesBelowTags {

  /** Damage-type tags: fatal-damage passthrough, wound classification and contact-heat tiers. */
  object DamageTypes {
    ...
    /** Damage types that supply the strong direct-contact heat tier: standing in fire and the
      * burning DOT (see `temperature.HeatDamageContribution`). Datapacks may extend the tier.
      */
    val HeatStrong: TagKey[DamageType] =
      TagKey.create(Registries.DAMAGE_TYPE, CasualtiesBelowApi.id("heat_strong"))

    /** Damage types that supply the normal contact heat tier: magma blocks (sneak-exempt by
      * vanilla) and campfires (see `temperature.HeatDamageContribution`). Datapacks may extend.
      */
    val HeatNormal: TagKey[DamageType] =
      TagKey.create(Registries.DAMAGE_TYPE, CasualtiesBelowApi.id("heat_normal"))
  }

  /** Item tags: wound weapon classes, shelf-drying inputs, food discomfort tiers and ambient
    * heat/cold sources.
    */
  object Items {

    /** Melee weapons that cut skin open (swords, axes by default). */
    val SharpMelee: TagKey[Item] =
      TagKey.create(Registries.ITEM, CasualtiesBelowApi.id("sharp_melee"))

    /** Plant materials that vanilla shelves dry into fiber cloth. */
    val DriesToFiberCloth: TagKey[Item] =
    ...
    /** Carried items that supply the weak ambient heat tier: small open flames and strong heat
      * thematics (torches, lanterns, blaze powder). Ambient source tiers run 1 = weak, 2 = strong,
      * 3 = extreme/freezing (see `temperature`). Datapacks may extend the tier.
      */
    val HeatSource1: TagKey[Item] =
      TagKey.create(Registries.ITEM, CasualtiesBelowApi.id("heat_source_1"))

    /** Carried items that supply the strong ambient heat tier: reliably-lit campfires, carried
      * magma and blaze rods. Datapacks may extend the tier.
      */
    val HeatSource2: TagKey[Item] =
    ...
    /** Carried items that supply the weak ambient cold tier: meltable ice, snow and carried water.
      ...
      */
    val ColdSource1: TagKey[Item] =
      TagKey.create(Registries.ITEM, CasualtiesBelowApi.id("cold_source_1"))

    /** Carried items that supply the strong ambient cold tier (packed ice). Datapacks may extend
      * the tier.
      */
    val ColdSource2: TagKey[Item] =
    ...
    /** Carried items that supply the freezing ambient cold tier (powder snow buckets, blue ice).
      * Datapacks may extend the tier.
      */
    val ColdSource3: TagKey[Item] =
      TagKey.create(Registries.ITEM, CasualtiesBelowApi.id("cold_source_3"))
  }

  /** Entity-type tags: attacker classification. */
  object Entities {
  ...
  object Blocks {
    ...
    /** Blocks whose breaking raises no dust at all (leaves, wool, wood, glass, ice; see
      * `hygiene.DirtinessSources`). Membership in [[DiggableDirty]] wins over this tag.
      */
    val DiggableDustless: TagKey[Block] =
    ...
    /** Blocks that radiate the extreme ambient heat tier (lava, still or cauldron-contained).
      * Datapacks may extend the tier.
      */
    val HeatSource3: TagKey[Block] =
      TagKey.create(Registries.BLOCK, CasualtiesBelowApi.id("heat_source_3"))

    /** Blocks that radiate the weak ambient cold tier: meltable or transient cold (ice, snow
      * layers, frosted ice). Datapacks may extend the tier.
      */
    val ColdSource1: TagKey[Block] =
      TagKey.create(Registries.BLOCK, CasualtiesBelowApi.id("cold_source_1"))

    /** Blocks that radiate the strong ambient cold tier: permanent ice and solid snow. Datapacks
      * may extend the tier.
      */
    val ColdSource2: TagKey[Block] =
      TagKey.create(Registries.BLOCK, CasualtiesBelowApi.id("cold_source_2"))

    /** Blocks that radiate the freezing ambient cold tier: powder snow, loose or
      * cauldron-contained. Datapacks may extend the tier.
      */
    val ColdSource3: TagKey[Block] =
