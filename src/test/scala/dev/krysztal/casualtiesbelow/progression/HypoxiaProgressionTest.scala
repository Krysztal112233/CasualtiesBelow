...
final class HypoxiaProgressionTest {
  ...
  def terminalExposureSynchronizesAtSemanticMilestones(): Unit = {
    val started = HypoxiaProgression.advance(0, 0.0, breathingBlocked = true, 160)
    assertEquals(1, started.exposureTicks)
    assertTrue(started.changed)
    assertTrue(started.syncDue)
    assertFalse(started.fatal)

    val ordinary = HypoxiaProgression.advance(1, 0.0, breathingBlocked = true, 160)
    assertEquals(2, ordinary.exposureTicks)
    assertTrue(ordinary.changed)
    assertFalse(ordinary.syncDue)

    val oneSecond = HypoxiaProgression.advance(19, 0.0, breathingBlocked = true, 160)
    assertEquals(20, oneSecond.exposureTicks)
    assertTrue(oneSecond.syncDue)

    val fatal = HypoxiaProgression.advance(159, 0.0, breathingBlocked = true, 160)
  ...
  @Test
  def restoredBreathingResetsAndSynchronizesExposureImmediately(): Unit = {
    val reset = HypoxiaProgression.advance(87, 0.0, breathingBlocked = false, 160)
    assertEquals(0, reset.exposureTicks)
    assertTrue(reset.changed)
    assertTrue(reset.syncDue)
    assertFalse(reset.fatal)

    val alreadyReset = HypoxiaProgression.advance(0, 0.0, breathingBlocked = false, 160)
  ...
  def positiveBloodOxygenCannotStartTerminalExposure(): Unit = {
    val preserved = HypoxiaProgression.advance(0, 0.01, breathingBlocked = true, 160)
  ...
  def aOneTickConfiguredDurationStillPublishesItsFatalEndpoint(): Unit = {
    val fatal = HypoxiaProgression.advance(0, 0.0, breathingBlocked = true, 0)
